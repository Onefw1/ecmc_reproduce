...
class MMDADataset(Dataset):
    ...
    def _pad_feature(self, feature: np.ndarray, max_len: int) -> np.ndarray:
        ...
        if len(feature) > max_len:
            return feature[:max_len] # 太长 → 截断
        elif len(feature) < max_len:
            return np.pad(feature, ((0, max_len - len(feature)), (0, 0)))  # 太短 → 用0填充
        return feature
...
class MultilabelBalancedSampler:
    ...
    多标签平衡采样器，用于在多标签分类任务中根据指定策略对数据进行均衡采样。
    """
    def __init__(self, dataset: MMDADataset, batch_size: int, strategy: str = "emotion"):
        ...
        Args:
            dataset (MMDADataset): 多模态数据集对象，包含情感、认知等标签信息。
            batch_size (int): 每个批次包含的总样本数。
            strategy (str): 平衡采样的具体策略。
...
#.npy 文件读取后存入 data["audio"] 字典，然后被打包成 batch 传入模型。
def collate_multimodal_batch(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    # batch 是一个列表，包含多个 data 字典
    # [data1, data2, data3, ...]
