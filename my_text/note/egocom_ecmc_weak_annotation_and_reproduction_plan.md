...
下一步：执行完整 1 epoch（978 step）的资源安全 Stage 1 训练，使用 batch size 4，checkpoint 间隔设为 978 step，并关闭 `save_last` 以避免重复保存。`train.py` 已支持通过环境变量设置 batch size、step 数、worker 数、checkpoint 目录、checkpoint 间隔和是否保存 `last.ckpt`。
