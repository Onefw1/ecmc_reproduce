...
下一步：从 `stable_fp32_epoch2` 的稳定 checkpoint 恢复，完成总计 5 个 epoch 的资源安全 Stage 1 训练。训练入口支持 `ECMC_RESUME_CKPT` 指定恢复路径；完成后再准备 Stage 2 所需的 LLaMA 权重与适配后的 emotion caption prompt。
