...
下一步：执行完整 1 epoch（977 step）的资源安全 Stage 1 训练，使用 batch size 4，checkpoint 间隔设为 977 step，并关闭 `save_last` 以避免重复保存。`train.py` 已支持通过环境变量设置 batch size、step 数、worker 数、checkpoint 目录、checkpoint 间隔和是否保存 `last.ckpt`。
...
当前正在完成 Stage 1 的稳定完整 epoch 训练。首次完整 epoch 在 FP16 mixed precision 下出现了数值 NaN，已定位为对比损失的半精度数值不稳定，并改为 FP32 稳定计算与资源安全训练配置。下一步是在该配置下完成无 NaN 的完整 epoch，再开展 Stage 2 emotion caption 实验。整个实验应表述为 EgoCom 上的 ECMC-inspired 弱监督多模态对话实验，而非原论文临床数据集上的严格复现。
